        ...
        <div class="container  mx-auto relative h-full pt-[10px] px-[150px] pb-[60px]">
            ...
            <div class="blog-actions flex mt-[32px]">
                <button @click="updateBlog"
                    class="w-fit mr-[16px] no-underline flex items-center justify-center py-[12px] px-[24px] text-white rounded-[20px] border-none bg-[#303030] uppercase focus:outline-none hover:bg-[rgba(48,48,48,0.7)] text-[14px] transition-all duration-500 hover:cursor-pointer">Save
                    Changes
                </button>
                <router-link :to="{ name: 'blogpreview' }"
                    class="router-button w-fit no-underline flex items-center justify-center py-[12px] px-[24px] text-white rounded-[20px] border-none bg-[#303030] uppercase focus:outline-none hover:bg-[rgba(48,48,48,0.7)] text-[14px] transition-all duration-500">
...
export default {
    ...
    async mounted() {
        ...
        console.log(this.blogPhotoFile)
    },
    computed: {
        profileId() {
        ...
        newBlogPhotoFile() {
            return this.$store.state.newBlogPhotoFile;
        },
    },
    methods: {
        fileChange() {
            ...
            this.$store.commit("setNewBlogPhotoFile", file); // Commit the new file to the store
            const blogPhotoFileChange = "";
            this.$store.commit("setBlogPhotoFile", blogPhotoFileChange); // remove the old the file to the store
            console.log(this.blogPhotoFile)
        },
        openPreview() {
        ...
        //                 const fileNameReset = null;
        //                 this.$store.commit("fileNameChange", fileNameReset);
        //                 this.$store.commit("updateBlogTitle", fileNameReset);
        //                 this.$store.commit("newBlogPost", fileNameReset)
        ...
        //         try {
        //             this.loading = !this.loading;
        //             const blogDocRef = doc(db, "blogPosts", this.routeID);
        //             const updatedBlogData = {
        //                 blogTitle: this.blogTitle,
        //                 blogHTML: this.blogHTML,
        //                 blogCoverPhoto: this.currentBlog[0].blogCoverPhoto,
        //                 createdAt: new Date(),
        //                 profileId: this.profileId
        //             };
        ...
        async updateBlog() {
            if (this.blogTitle.length !== 0 && this.blogHTML.length !== 0) {
                try {
                    ...
                    if (this.blogPhotoFile) {
                        // Use existing cover photo details
                        blogData = {
                            blogTitle: this.blogTitle,
                            blogHTML: this.blogHTML,
                            createdAt: new Date(),
                        ...
                        // Upload new cover photo logic
                        const storageReference = storageRef(storage, `blogCovers/${this.blogCoverPhotoName}`);
                        const metadata = { contentType: this.newBlogPhotoFile.type };
                        const snapshot = await uploadBytes(storageReference, this.newBlogPhotoFile, metadata);
                        const downloadURL = await getDownloadURL(snapshot.ref);

                        // Update blogData with new cover photo details
                        blogData.blogCoverPhotoName = this.blogCoverPhotoName;
                        blogData.blogCoverPhoto = downloadURL;
                    ...
                    // Dispatch action to update post in Vuex state
                    await this.$store.dispatch("updatePost", this.routeID);
